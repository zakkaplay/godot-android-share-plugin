...
@onready var getShare: Share = $Share
...
func _on_share_texture_button_pressed() -> void:
	...
	getShare.share_texture(_texture_rect.texture, shared_title, shared_subject, _text_edit.text)


func _on_share_viewport_button_pressed() -> void:
	_print_to_screen("Sharing viewport")
	getShare.share_viewport(get_viewport(), shared_title, shared_subject, _text_edit.text)
